...
# Env-overridable so the portable Docker kit can point at mounted paths.
CANDIDATES = Path(os.environ.get("CANDIDATES_PATH", ROOT / "scripts" / "image_candidates.json"))
OUT = Path(os.environ.get("REVIEW_HTML_PATH", ROOT / "scripts" / "image_review.html"))
...
<style>
  ...
  .import{margin-left:auto;background:#23252e;color:#e7e7ea;border:1px solid #3a3b44;border-radius:8px;padding:9px 14px;font-weight:600}
  .import:hover{border-color:#DC2626}
  .export{background:#DC2626;color:#fff;border:0;border-radius:8px;padding:9px 16px;font-weight:600}
  ...
  .repline{display:flex;align-items:center;gap:7px;margin-top:12px;color:#cdd2da;font-size:12.5px;cursor:pointer}
  .repline strong{color:#f59e0b;font-weight:600}
  .repline input{accent-color:#f59e0b}
...
<header>
  ...
  <input type="file" id="importFile" accept="application/json,.json" style="display:none" onchange="importApprovals(this)">
  <button class="import" onclick="document.getElementById('importFile').click()" title="Restore selections from a previously exported approvals.json">&#8681; Import approvals.json</button>
...
const rep = JSON.parse(localStorage.getItem('tr_img_rep')||'{}'); // id -> true (image is a close-but-not-exact stand-in)
...
function candsHTML(p){
  ...
  h+=`<label class="repline"><input type="checkbox" ${rep[p.id]?'checked':''}
        onchange='toggleRep(${J(p.id)},this.checked)'> Close, but not the exact item —
        mark as <strong>representative image</strong> (shopper sees a &ldquo;may vary&rdquo; note)</label>`;
  h+=`<div class="refine">
...
function toggleRep(pid,on){ if(on) rep[pid]=true; else delete rep[pid]; localStorage.setItem('tr_img_rep',JSON.stringify(rep)); }
...
function exportApprovals(){
  const out=Object.entries(sel).filter(([,u])=>u).map(([id,url])=>({id,url,representative:!!rep[id]}));
  const blob=new Blob([JSON.stringify(out,null,2)],{type:'application/json'});
  const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='approvals.json';a.click();
}
function importApprovals(inp){
  const file=inp.files&&inp.files[0]; if(!file) return;
  const reader=new FileReader();
  reader.onload=()=>{
    let arr;
    try{ arr=JSON.parse(reader.result); }catch(e){ alert('Could not read that file — not valid JSON.\\n\\n'+e.message); inp.value=''; return; }
    if(!Array.isArray(arr)){ alert('That doesn’t look like an approvals.json (expected a JSON array of {id,url}).'); inp.value=''; return; }
    let matched=0, missing=0, repCount=0;
    arr.forEach(o=>{
      if(!o||!o.id) return;
      const id=o.id, url=o.url||'';
      sel[id]=url;                                   // restore the pick ("" = rejected)
      if(o.representative){ rep[id]=true; repCount++; } else { delete rep[id]; }
      if(url){
        const p=prod(id);
        if(p){
          if(!p.candidates) p.candidates=[];
          if(!p.candidates.some(c=>c.url===url))     // re-inject a pick that isn't in the current list (e.g. a pasted URL)
            p.candidates.unshift({url,thumb:url,page:url,host:hostOf(url),engine:'imported',resolution:'imported',w:0,h:0,score:9999});
          matched++;
        } else { missing++; }                        // id not on this gallery's product list — kept so a re-export still includes it
      }
    });
    save(); localStorage.setItem('tr_img_rep',JSON.stringify(rep));
    document.getElementById('wrap').innerHTML=''; render();
    inp.value='';
    alert(`Imported ${arr.length} entr${arr.length===1?'y':'ies'}.\\n`+
          `${matched} matched to products on this page`+
          (repCount?`\\n${repCount} marked representative`:'')+
          (missing?`\\n${missing} not found on this list (kept anyway)`:''));
  };
