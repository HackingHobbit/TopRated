...
def load_env():
    """Read Supabase URL + service key from .env.local (apply phase only)."""
    vals = {}
    if ENV_LOCAL.exists():
    ...
    for a in approvals:
        ...
        try:
            ...
            patch = urllib.request.Request(
                ...
                data=json.dumps({"image": public}).encode(), method="PATCH",
                headers={"apikey": key, "Authorization": f"Bearer {key}",
            ...
            print(f"  applied {pid}  {by_id.get(pid,{}).get('name','')[:40]}")
        except Exception as e:
